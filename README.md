...
扫描 classpath 按 Maven 编译语义包含 `compile` 和 `provided` 依赖及其传递依赖。若仍有框架类缺失，调用发现阶段会把该调用降级为 `REVIEW`，而不会因 `NoClassDefFoundError` 终止整次扫描。
...
- 在一张 `Bean 属性并集` 表中查看 Source Bean 和 Target Bean 的全部属性，每个属性只出现一次
- 在同一行核对两侧存在性、Source/Target 类型、声明类、映射关系和版本结论
